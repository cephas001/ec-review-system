<template>
  <div class="max-w-6xl mx-auto pb-24 md:pb-0 relative">
    <div class="sm:flex sm:items-center sm:justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-black">Merchandise Review</h1>
      </div>
    ...
    <div v-else-if="currentTab === 'review'">
      <MerchNoReviewMerch v-if="pendingApplications.length === 0" />
      ...
      >
        ...
        <div
          class="flex-1 bg-gray-50 p-4 md:p-6 flex flex-col items-center justify-center min-h-137.5 md:min-h-175"
        >
          <h4
          ...
          >
            <iframe
              ...
              class="w-full h-full min-h-137.5 md:min-h-175 border-0"
    ...
    >
      ...
      >
        ...
        <div
          class="flex-1 bg-gray-50 p-4 md:p-6 flex flex-col items-center justify-center min-h-[60vh] md:min-h-150"
        >
          <iframe
    ...
    <div
      v-if="showInfoPanel"
      class="fixed bottom-24 right-6 z-40 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden animate-fade-in-up"
    >
      <div class="bg-black text-white p-4">
        <h3 class="text-center text-sm">Merch Reference Info</h3>
      </div>
      <div class="p-5 overflow-y-auto max-h-[60vh]">
        <h4 class="text-xs font-bold text-black uppercase mb-4 pl-1">
          Packages & Pricing
        </h4>
        <ul class="space-y-3 mb-6 text-sm">
          <li
            class="flex justify-between items-center border border-gray-100 p-2 bg-gray-50 rounded-md"
          >
            <span class="text-black"
              ...
              ></span
            >
            <span class="font-bold text-black tracking-wide">₦10,000</span>
          </li>
          <li
            class="flex justify-between items-center border border-gray-100 p-2 bg-gray-50 rounded-md"
          >
            <span class="text-black"
              >Deluxe
              <span class="block text-xs text-gray-600 font-normal"
                >T-shirt, Jotter, Wristband</span
              ></span
            >
            <span class="font-bold text-black tracking-wide">₦8,000</span>
          </li>
          <li
            class="flex justify-between items-center border border-gray-100 p-2 bg-gray-50 rounded-md"
          >
            <span class="text-black"
              ...
              ></span
            >
            <span class="font-bold text-black tracking-wide">₦7,000</span>
          </li>
          <li
            class="flex justify-between items-center border border-gray-100 p-2 bg-gray-50 rounded-md"
          >
            <span class="text-black"
              >Minimum
              <span class="block text-xs text-gray-600 font-normal"
                >Jotter, Water Bottle</span
              ></span
            >
            <span class="font-bold text-black tracking-wide">₦3,500</span>
          </li>
          <li
            class="flex justify-between items-center border border-gray-100 p-2 bg-gray-50 rounded-md"
          >
            <span class="text-black">T-Shirt only</span>
            <span class="font-bold text-black tracking-wide">₦5,000</span>
          </li>
          <li
            class="flex justify-between items-center border border-gray-100 p-2 bg-gray-50 rounded-md"
          >
            <span class="text-black">Wristband only</span>
            <span class="font-bold text-black tracking-wide">₦1,000</span>
          </li>
        </ul>

        <h4 class="text-xs font-bold text-black uppercase mb-3 pl-1">
          Account Details
        </h4>
        <div class="bg-gray-50 p-2 rounded-lg border border-gray-100 text-sm">
          <p class="mb-1">
            <span class="text-gray-600">Bank: </span>
            <span class="font-bold text-black tracking-wide">MONIEPOINT</span>
          </p>
          <p class="mb-1">
            <span class="text-gray-600">Account: </span>
            <span class="font-bold text-black tracking-wide">9160822562</span>
          </p>
          <p>
            <span class="text-gray-600">Name: </span>
            <span class="font-bold text-black">KADIRI IYINOLUWA ESTHER</span>
          </p>
        </div>
      </div>
    </div>
...
const nextApp = () => {
  if (currentIndex.value < pendingApplications.value.length - 1) {
    currentIndex.value++;
    reviewComment.value = ""; // Clear comment when navigating
  }
};
...
const currentApp = computed(() => {
  ...
  // Fallback to max index if array shrinks below current index
...
// Generate the final API URL for the current pending application's image
const currentReceiptUrl = computed(() => {
  ...
  // Point to our Express backend proxy without the view parameter
...
const fetchApplications = async () => {
  loading.value = true;
  currentIndex.value = 0; // Reset index on refresh
  try {
...
const updateStatus = async (rowIndex, newStatus) => {
  ...
  try {
    // Note: We are sending the comment field in the body now.
    ...
    // Clear comment box for the next application
    reviewComment.value = "";

    // Automatically manage index so user is looking at the next logical card
    if (currentIndex.value >= pendingApplications.value.length) {
      currentIndex.value = Math.max(0, pendingApplications.value.length - 1);
    }
  } catch (err) {
...
<style>
/* Simple utility animation for the info panel pop-up */
@keyframes fadeInUp {
  from {
    opacity: 0;
